...
// build, so the `entitlements` table this used to read is gone — there is
// no PRO tier left to check. The one local user always has it.
...
// hasProEntitlement() above always returns true now, so this can never
// throw — kept as a thin wrapper (not deleted) so every call site below
// needs no change.
async function assertProEntitlement(
  ctx: MutationCtx,
  userId: string,
): Promise<void> {
  await hasProEntitlement(ctx, userId);
}

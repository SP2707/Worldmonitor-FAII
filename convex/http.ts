...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    // UNIFIED-ENGINE CHANGE: no Clerk/Convex auth backend exists in this
    // build, so ctx.auth.getUserIdentity() would return null on every call
    // (no JWT provider is configured — see convex/auth.config.ts) and this
    // gate would permanently 401 the one local user. The downstream
    // userPreferences.setPreferences mutation resolves the caller via
    // convex/lib/auth.ts::resolveUserId(), which always returns the fixed
    // local user id now, so there is nothing left to authenticate here.

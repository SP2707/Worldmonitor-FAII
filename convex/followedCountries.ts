...
// build, so the `entitlements` table this used to read is gone. There is no
// free tier left to cap against — the one local user always gets the PRO
// watchlist limit. Tier 3 matches the synthetic full-access entitlement
// object used across server/_shared/entitlement-check.ts for consistency.

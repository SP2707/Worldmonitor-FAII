...
// build, so the `entitlements` table this function used to check is gone —
// there is no PRO tier left to gate. Notifications are unconditionally
// available to the one local user. Kept as a no-op (rather than deleted) so
// every existing call site below needs no change beyond staying awaited.

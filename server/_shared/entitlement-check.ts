...
export function getRequiredTier(pathname: string): number | null {
  // UNIFIED-ENGINE CHANGE: no Clerk/Convex/Dodo billing backend exists in this
  // local, single-user build, so no route is tier-gated anymore. ENDPOINT_ENTITLEMENTS
  // above is left in place only as a historical record of which routes used to
  // require a paid tier — it is no longer consulted.
  void pathname;
  return null;
}
...
// at all — there are no accounts, no subscriptions, and no tiers, so there is
// nothing left to look up. Every caller (the one local user) gets full access
// to every feature unconditionally. This replaces what used to be a
// Redis-cached, Convex-backed, negative-cached, request-coalesced remote
// lookup (the machinery for that — _getEntitlementsImpl, _inFlight,
// _unavailableUntil, rememberVerificationUnavailable, unavailableEntitlements
// below — is left in place but no longer called from here; it's dead code
// kept only because other exports in this file still reference its types,
// and removing it fully is follow-up cleanup, not a functional requirement).
export async function getEntitlements(userId: string): Promise<CachedEntitlements | null> {
  ...
  return {
    ...
    validUntil: 8_640_000_000_000_000, // JS max safe date (year 275760) — never expires
  };
}

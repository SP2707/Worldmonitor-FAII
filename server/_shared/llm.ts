...
export const callLlmReasoning = (opts: Omit<LlmCallOptions, 'providerOrder' | 'modelOverrides'>) =>
  // Unified-engine default: Groq (GROQ_API_KEY) is the shared text-to-text
  // provider. Override per-call via LLM_REASONING_PROVIDER if a stage needs
  // OpenRouter/DeepSeek-class reasoning capability Groq's hosted models don't match.
  callLlmProfile({ enableReasoning: true, ...opts }, 'LLM_REASONING_PROVIDER', 'LLM_REASONING_MODEL', 'groq');
...
export function callLlmReasoningStream(opts: LlmStreamOptions): ReadableStream<Uint8Array> {
  ...
  // Unified-engine default: Groq, matching callLlmReasoning above.

...
// Unified-engine change: Groq (GROQ_API_KEY) is now tried ahead of
// OpenRouter, reversing the order #4944 originally set (OpenRouter's
// DeepSeek V4 Flash ahead of Groq, for summarization quality on harder
// articles). The RPC honors the client-supplied provider, so this
// try-order decides which model summarizes — if article-summary quality
// regresses on complex pieces, that regression traces to this line, not
// the server. Revert this ordering (groq/openrouter swapped) if so.

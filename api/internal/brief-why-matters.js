...
var require_llm_model_policy = __commonJS({
  "scripts/lib/llm-model-policy.cjs"(exports, module) {
    "use strict";
    var OPENROUTER_FREE_PRIMARY_MODEL2 = "google/gemma-4-26b-a4b-it:free";
    var OPENROUTER_FREE_BACKUP_MODEL2 = "openai/gpt-oss-20b:free";
    var GROQ_DEFAULT_MODEL2 = "openai/gpt-oss-20b";
    var OPENROUTER_PROVIDER_ROUTING2 = {
      ignore: ["baidu", "alibaba", "deepseek", "siliconflow", "streamlake", "novita"],
      sort: "throughput"
    };
    module.exports = {
      GROQ_DEFAULT_MODEL: GROQ_DEFAULT_MODEL2,
      OPENROUTER_FREE_BACKUP_MODEL: OPENROUTER_FREE_BACKUP_MODEL2,
      OPENROUTER_FREE_PRIMARY_MODEL: OPENROUTER_FREE_PRIMARY_MODEL2,
      OPENROUTER_PROVIDER_ROUTING: OPENROUTER_PROVIDER_ROUTING2
    };
  }
});
...
var AND_JOINED_ACRONYM_VARIANTS = ACRONYM_EXPANSIONS.flatMap((group) => group).map((variant) => variant.toLowerCase().split(/\s+/)).filter((tokens) => tokens.includes("and"));
...
var import_llm_model_policy = __toESM(require_llm_model_policy(), 1);
...
} = import_llm_model_policy.default;
var OPENROUTER_BLOCKED_PROVIDERS = OPENROUTER_PROVIDER_ROUTING.ignore;
...
function getProviderCredentials(provider, overrides = {}) {
  ...
  const openRouterDefaultModel = OPENROUTER_DEFAULT_MODELS[provider];
...
var OPENROUTER_FREE_ATTEMPT_TIMEOUT_MS = 8e3;
var INDEPENDENT_FALLBACK_RESERVE_MS = 5e3;
function isOpenRouterProvider(provider) {
  return provider === "openrouter" || provider === "openrouter-free" || provider === "openrouter-free-backup";
}
...
${sanitized}` },
  ...
  try {
    for (let providerIndex = 0; providerIndex < providers.length; providerIndex += 1) {
      ...
      const hasIndependentFallback = isOpenRouterProvider(providerName) && providers.slice(providerIndex + 1).some((laterProvider) => {
        const laterCreds = getProviderCredentials(laterProvider, {
          model: modelOverrides?.[laterProvider],
          enableReasoning
        });
        return laterCreds !== null && new URL(laterCreds.apiUrl).origin !== new URL(creds.apiUrl).origin;
      });
      const reservedMs = hasIndependentFallback ? Math.min(INDEPENDENT_FALLBACK_RESERVE_MS, Math.max(0, remainingMs - 1)) : 0;
      ...
      const freeAttemptCapMs = providerName === "openrouter-free" || providerName === "openrouter-free-backup" ? OPENROUTER_FREE_ATTEMPT_TIMEOUT_MS : modelAttemptMs;

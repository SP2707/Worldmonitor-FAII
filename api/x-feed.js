...
var VARY_CREDENTIAL = "Origin, Cookie, X-WorldMonitor-Key, X-Api-Key, Authorization";
...
function normalizeXFeed(parsed) {
  const rawPosts = Array.isArray(parsed.posts) ? parsed.posts : Array.isArray(parsed.items) ? parsed.items : [];
  const items = rawPosts.map(normalizeXPost).filter((item) => item.contentState !== "deleted");
  const coverage = {

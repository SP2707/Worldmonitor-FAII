...
async function quotaHandler(req, deps) {
  ...
  const limit = onFreeAllowance ? FREE_ACCOUNT_CALLS_PER_DAY : resolveDailyLimit(planDailyLimit);
  const key = onFreeAllowance ? freeAccountCallsKey(userId, now.getTime()) : dailyCounterKey(userId, now);
  let raw = null;
  try {

...
if [ -f "$OVERRIDE" ]; then
  ...
  grep -E '^\s+[A-Z_]+:' "$OVERRIDE" \
    ...
    | sed 's/: */=/' \
    | sed "s/[\"']//g" \
    | grep -E '^(NASA_FIRMS|GROQ|AISSTREAM|FRED|FINNHUB|EIA|ACLED_ACCESS_TOKEN|ACLED_EMAIL|ACLED_PASSWORD|CLOUDFLARE|AVIATIONSTACK|OPENAQ_API_KEY|WAQI_API_KEY|OPENROUTER_API_KEY|LLM_API_URL|LLM_API_KEY|LLM_MODEL|OLLAMA_API_URL|OLLAMA_MODEL|WEB_SEARCH_URL|WEB_SEARCH_API_KEY|WEB_SEARCH_MAX_RESULTS|WEB_SEARCH_REGIONS|BRAVE_API_KEYS)' \
